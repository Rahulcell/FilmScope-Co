...
async function fetchMovies() {
  const response = await fetch(`https://rahulcell.github.io/IMDB-project/db.json`);
  ...
  moviesToShow.forEach((movie) => {
    ...
    movieCard.innerHTML = `
      ...
      <button class="bookmark-btn" data-movie-id="${movie.id}" title="Add to Favorites">
        <i class="bi ${
          favorites.includes(movie.id) ? "bi-bookmark-fill" : "bi-bookmark"
        }"></i>
      </button>
    `;
...
onAuthStateChanged(auth, async (user) => {
  if (user) {
    ...
    logoutBtnContainer.style.display = 'block'; // Show Logout button container
    ...
    logoutBtnContainer.style.display = 'none'; // Hide Logout button container
    favorites = [];
    renderFavorites();
  }
});
